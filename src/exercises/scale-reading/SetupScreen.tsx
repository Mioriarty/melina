...
import { ROUND_LENGTHS, type ScaleReadingSettings } from './settings'

export interface SetupScreenProps {
  settings: ScaleReadingSettings
  onChange: (settings: ScaleReadingSettings) => void
  onStart: () => void
 ...
 *
 * No key signature section, unlike the interval setups: a scale is always
 * written keyless, so the mode has to be read from the accidentals in front
 * of the notes. The tonic takes its place, because on a keyless staff the
 * tonic is what decides how many accidentals there are to read.
 */
export function SetupScreen({ settings, onChange, onStart, onBack }: SetupScreenProps) {
  ...
  function toggle<K extends 'clefs' | 'modes' | 'tonics'>(
    field: K,
    value: ScaleReadingSettings[K][number],
  ) {
      ...
      <div className="pb-page mx-auto w-full max-w-2xl px-4 pt-4 sm:px-6">
        <header className="mb-6">
          ...
          <h1 className="mt-1 text-title">
            {t('curriculum:categories.scales.exercises.reading.title')}
          </h1>
          <p className="mt-2 leading-relaxed text-ink-muted">
            {t('exercise:scales.reading.setupBlurb')}
          </p>
        </header>
